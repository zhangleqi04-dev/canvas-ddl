...
flowchart TB
    ...
    subgraph ING["文档 ingestion：查询前完成"]
        ...
        DP --> EX["DeadlineExtractor<br/>提出 DeadlineCandidate"]
        EX --> VA["DeadlineValidator<br/>独立验证原文、日期与课程绑定"]
        REG["人工维护的来源审批配置<br/>课程、期间、版本及更新授权"] -.-> VA
    end

    VA --> DB["已批准文档证据库<br/>保存原文、候选与验证审核"]
...
Codex 负责理解和呈现，DeadlineService 负责调度，引擎负责最终日期和计数。
...
Codex 不计算相对日期；TimeRangeResolver 使用实际时钟验证并计算，结果附原始意图和时间边界。
