...
- Canvas structured records **and validated trusted course documents** form evidence.
...
```text
trusted course document → DocumentParser → StructuralChunker → LightSemanticPrefilter
...
must be rechecked by Python. Do not move document trust, operator-approved period, source
...
Canvas API and operator trust are separate authority modes. Files, Syllabus and published
Pages returned by authenticated, course-scoped Canvas endpoints register automatically as
`canvas_api` after exact origin, course path, resource ID and hash checks. They do not wait
for human approval or a manually entered course period. Exam queries still check every
accessible supported course document; other complete positive queries may skip refresh.
Never send Canvas auth to storage hosts, persist signed download URLs or use remote filenames
as local paths. Download-host permission is transport policy, not evidence authority.

External URLs and manually supplied local course documents use `operator` authority. Only
this path may create pending records and require a real approver, verified course/type, hash
and valid teaching period. Filename, self-description, LLM confidence and arbitrary URLs do
not grant trust. Codex must never invent an approval identity. Provisional external/manual
content stays outside canonical facts.

Every confirmed document source retains authority mode, URL, ID/name/hash, format-specific
location, exact source evidence/date expression, normalized date/time, ingestion time and
validation audit. Queries revalidate persisted candidates and do not open source documents.
Canvas resources update automatically only after the same authenticated scoped identity is
observed; keep version history. Missing/failed current versions set `refresh_blocked` and
withhold old facts. External/manual resources cannot inherit trust by filename. Unknown
network/permission state is partial; unchanged metadata/content must reuse artifacts.
...
- Week 7 Friday/第7周 stays unresolved at ingestion. Only TeachingWeekResolver may
  ...
  Conflicts remain unresolved. Never let Codex resolve Week N. Codex may normalize a
  literal date or AM/PM in schema v2, but Python must verify the source-compatible mapping;
  a missing year is allowed only when full-document context has one unambiguous year.
- A mapped relative week is a non-canonical ReferenceDeadline. Keep
  ...
  document/calendar provenance, external-source trust risk, confidence and precision.
...
- One chunk may contain several dates/events; Codex binds one exact evidence/date
  span per logical event, emits ISO normalized_date/time separately, and Python validates
  each source-compatible mapping independently. Statistical tests,
...
- For this user's requested best-effort presentation, clear trusted document
...
`courses/` owns resolution; `documents/` owns trusted registry, parser, chunker,
...
- No-keyword semantic deadline, audited statistical-test negative, multi-event/date
  chunk, normalized numeric/ordinal/AM-PM dates, incomplete review withholding,
  fabricated span/date mapping rejection and exact
...
Authenticated Canvas course resources register automatically and use the main evidence store.
Untrusted external/manual documents may be parsed/audited in a separate provisional store;
their operator course/period trust remains absent and validation cannot confirm them. Never
