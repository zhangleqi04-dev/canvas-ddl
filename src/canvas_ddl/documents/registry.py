"""Operator-maintained trust configuration, never supplied by an extractor."""
import json
import re
...
class OfficialDocumentRegistry:
    ...
    def list_documents(self, course_ids=None) -> tuple[OfficialDocument, ...]:
        ...
        try:
            ...
            for row in rows:
                ...
                approver = row["approved_by"].strip()
                ...
                if (source.scheme != "https" or source.hostname not in self.hosts or source.username or source.password
                        ...
                        or not approver or approver.casefold() in ("codex", "llm", "extractor", "auto")
                ...
                if document_format(row["document_name"]) is None or document_format(local.name) is None:
                    raise ValueError
                result.append(OfficialDocument(identifier, str(row["course_id"]), row["course_code"], row["course_name"],
                                               ...
                                               row.get("auto_refresh", False), row.get("refresh_blocked", False)))
        except (ValueError, KeyError, TypeError, AttributeError, OSError):
            raise ApplicationError("INVALID_DOCUMENT_REGISTRY", "The operator-approved official document registry is invalid.") from None
    ...
    def get(self, document_id):
        ...
        if not match:
            raise ApplicationError("DOCUMENT_NOT_REGISTERED", "Only operator-approved official course documents may be ingested.")
        return match[0]
